...
class SimpleCarAgent(Agent):
    def __init__(self, history_data=int(50000)):
        ...
        self.neural_net = Network([self.rays + 4,
                                   (self.rays + 4) * 2,# внутренние слои сети: выберите, сколько и в каком соотношении вам нужно
    ...
    def show_weights(self):
        ...
        np.set_printoptions(threshold=None)#np.nan)
        return repr(params)
    ...
    def receive_feedback(self, reward, train_every=50, reward_depth=15):#7
        """
        ...
        if not self.evaluate_mode and (len(self.reward_history) >= train_every) and not (self.step % train_every):
            ...
            self.neural_net.SGD(training_data=train_data, epochs=15, mini_batch_size=train_every, eta=0.5)
